...
Write-Host "OutcomeIQ Day 6 recommendation full verification" -ForegroundColor Cyan
...
try {
    ...
    $EnvironmentStatus = @(
        $GitStatus | Where-Object { $_ -match '(^|\s)backend[\\/]\.env$' }
    )
    ...
    if ($TableExitCode -ne 0 -or "ALL REQUIRED TABLES EXIST" -notin $TableLines) {
    ...
    $CurrentStep = "recommendation API smoke check"
    Invoke-RepositoryScript "scripts\smoke_recommendation_api.ps1"
    $Succeeded = $true
}
catch {
    Write-Host "DAY 6 RECOMMENDATION FULL VERIFY FAILED" -ForegroundColor Red
...
if ($Succeeded) {
    Write-Host "DAY 6 RECOMMENDATION FULL VERIFY PASSED" -ForegroundColor Green
    exit 0
}
