...
Write-Host "OutcomeIQ Day 6 dashboard full verification" -ForegroundColor Cyan
...
try {
    ...
    $CurrentStep = "dashboard API smoke check"
    Invoke-RepositoryScript "scripts\smoke_dashboard_api.ps1"
    $Succeeded = $true
}
catch {
    Write-Host "DAY 6 DASHBOARD FULL VERIFY FAILED" -ForegroundColor Red
...
if ($Succeeded) {
    Write-Host "DAY 6 DASHBOARD FULL VERIFY PASSED" -ForegroundColor Green
    exit 0
}
