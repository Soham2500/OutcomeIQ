...
|---|---|---|---|---|
| `POST /api/v1/auth/register` | Creates a user. No authentication required. | `email`, `password`, `display_name` | User identity, access token, refresh token, expiry metadata | Valid email; strong password; bounded display name. `409 email_already_registered`, `422 validation_error` |
